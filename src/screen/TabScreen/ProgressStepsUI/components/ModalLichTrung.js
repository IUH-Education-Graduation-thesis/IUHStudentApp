...
const ModalLichTrung = ({ data, isVisible, onClose, isTrung }) => {
    ...
    useEffect(() => {
        const ids = lichHocs.map((item, index) => index)
    ...
    const _renderContent = (item) => {
        return !item?.lichHocs.map(lh => {
            console.log("item", JSON.stringify(lh.nhomThucHanh, null, 3));

            const _isThucHanh = !lh.isLyThuyet;
            const _titleLicHoc = _isThucHanh ? 'TH' : 'LT';
            const _thu =
                lh?.ngayHocTrongTuan === 8
                    ? 'Chủ nhật'
                    : `Thứ ${lh?.ngayHocTrongTuan}`;
            ...
            return (
                <View
                    ...
                    <Text>{`Giảng viên: ${lh?.giangVien?.hoTenDem ? lh?.giangVien?.hoTenDem : "Giảng viên tạm"} ${lh?.giangVien?.ten ? item?.giangVien?.ten : ""}`}</Text>
            ...
            <View
                ...
                <View>
                    ...
                    <ScrollView style={{ marginTop: 10, paddingHorizontal: 10 }}>
                        {isTrung ? renderListLichTrung() : <Text style={{ fontSize: 20, fontWeight: '600' }}>Không có lịch trùng</Text>}
                    </ScrollView>
                </View>
